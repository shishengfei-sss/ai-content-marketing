...
todos:
  ...
  - id: phase7-deploy
    ...
    status: pending
  - id: phase8-future
    content: 二期：多行业 pack、个人提示词增强、Frappe CRM Lead 对接
    status: pending
isProject: false
---
...
帮小财务公司：**用 AI 生成营销内容 → 人工审核 → 公众号自动发；小红书/抖音生成后导出**；以后可加行业、知识库、个人提示词、接 Frappe CRM。
...
与 [e:\frappe-crm](e:\frappe-crm) **并列、独立 Git 仓库**；CRM 集成放在二期。

**需求规格文档：** [ai-content-marketing-需求规格.md](./ai-content-marketing-需求规格.md)（FR/NFR、验收清单、状态机）
...
3. **Frappe CRM 线索** — 小程序表单 → Webhook → [e:\frappe-crm\crm-app](e:\frappe-crm\crm-app) 创建 Lead  
4. **订阅号降级** — 仅生成+复制，不自动发  
...
- MVP **不修改** [e:\frappe-crm](e:\frappe-crm)  
- 二期在 `apps/api` 增加 `integrations/frappe_crm.py`，POST Lead 到 `crm.localhost` 或生产 CRM 地址  
