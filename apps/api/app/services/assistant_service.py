...
DEFAULT_COMPLIANCE = """1. 内容必须合规，不得夸大承诺或误导用户（禁用「保证」「百分百」「零风险」「有底气的保障」等绝对化/准绝对化表述）
2. 必须包含免责声明（图文文末；视频脚本在最后一镜口播/字幕中体现）：{disclaimer}
3. 免责声明须单独成段，放在文末；禁止与「联系我们」等行动号召揉在同一句/同一段
4. 禁止自相矛盾收尾，例如先推销「有底气/放心保障」再写「我们郑重承诺绝不做绝对化承诺」
5. 语气{tone}，适合目标受众阅读
6. 直接输出正文内容，不要输出 JSON 或多余解释"""
