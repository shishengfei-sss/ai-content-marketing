/** CRM 高级筛选：字段、操作符与 payload 构建（Web / H5 共用逻辑） */
...
export function cloneFilters(filters) {
  return {
    ...
    conditions: (filters?.conditions || []).map((c) => ({ ...c })),
  }
}
...
export function serializeFiltersParam(filters) {
  if (!hasActiveFilters(filters)) return undefined
  return JSON.stringify(buildFiltersPayload(filters.conditions ? filters : { conditions: filters }))
}
...
export function attachFieldTypes(conditions, fields) {
  const map = Object.fromEntries((fields || []).map((f) => [f.field_key, f]))
  return (conditions || []).map((c) => ({
    ...c,
    _field_type: map[c.field_key]?.field_type,
  }))
}
