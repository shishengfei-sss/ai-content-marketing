...
const ALL_COLS = [
  ...
  { key: 'lesson_count', label: '课时数', defaultOn: true },
...
async function exportCsv(mode) {
  ...
  try {
    ...
    await submitShopExport(
      '/api/v1/shop/columns/export',
      body,
      '/api/v1/shop/columns/export-tasks',
      'shop-columns.csv',
      total.value,
    )
  } catch (e) {
...
<template>
  <div v-loading="loading" data-testid="shop-columns">
    ...
    <el-table :data="items" border stripe size="small" style="margin-top: 12px">
      ...
      <el-table-column v-if="colKey === 'lesson_count'" prop="lesson_count" label="课时数" width="90" />
