...
import { CUSTOMER_STATUS_OPTIONS } from '../../utils/entityForm'

const router = useRouter()
const route = useRoute()
const auth = useAuthStore()
const { listColumns, fields, loadListColumns, loadColumnSettingsDraft, saveListColumns, formatCell, loadSchema } = useEntitySchema('customer')
...
const statusOptions = computed(() => {
  ...
  return Array.isArray(fromSchema) && fromSchema.length ? fromSchema : CUSTOMER_STATUS_OPTIONS
})
...
function clearFilters() {
  ...
  if (route.query.view_id) {
    router.replace({ path: '/crm/customers' })
  }
  page.value = 1
  loadCustomers()
}
...
async function loadViews() {
  try {
    const { data } = await crmApi.listViews('customer')
...
async function loadCustomers() {
  ...
  try {
    ...
    if (data.list_fields?.length) {
      listColumns.value = data.list_fields.filter((c) => c.visible !== false)
...
async function submitSaveView() {
  ...
  try {
    ...
    navigateToView(router, '/crm/customers', data.id)
    loadCustomers()
  } catch (e) {
    ElMessage.error(e.message || '保存失败')
  }
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm(`确定删除客户「${row.company_name}」？`, '删除')
...
<template>
  <div class="page-card">
    <CrmListToolbar
      ...
      :show-filter-hint="hasTemporaryFilter && !loading"
    ...
    <CrmImportDialog v-model:visible="importVisible" entity-type="customer" @done="loadCustomers" />
    <CrmImportHistory v-model:visible="importHistoryVisible" entity-type="customer" />
  </div>
</template>
