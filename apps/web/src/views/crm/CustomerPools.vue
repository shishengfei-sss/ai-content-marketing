...
const canClaim = () => hasPermission(auth.permissions, 'crm.customer.edit')
const canManagePools = () => hasPermission(auth.permissions, 'crm.customer.edit')
...
async function loadPools() {
  ...
  try {
    const { data } = await crmApi.listCustomerPools()
...
async function loadCustomers() {
  ...
  try {
    const { data } = await crmApi.listCustomerPoolCustomers(activePoolId.value)
    customers.value = Array.isArray(data) ? data : []
  } catch (e) {
    ElMessage.error(formatApiError(e, '加载公海客户失败'))
    customers.value = []
  } finally {
...
async function claim(row) {
  ...
  try {
    await ElMessageBox.confirm(`认领客户「${row.company_name}」？认领后将归到你名下。`, '认领客户', {
      type: 'info',
      confirmButtonText: '认领',
    })
    claimingId.value = row.id
    await crmApi.claimCustomerFromPool(activePoolId.value, row.id)
...
function goCustomer(row) {
  router.push({ path: `/crm/customers/${row.id}`, query: { from: 'customer-pools' } })
}
...
<template>
  <div class="page-card">
    <div class="page-head">
      <div>
        <h2>客户公海</h2>
        <p class="hint">查看各公海池中待认领客户；认领后进入「我的客户」跟进。</p>
      </div>
      <el-button v-if="canManagePools()" @click="router.push('/settings/customer-pools')">
        管理公海池
      </el-button>
    </div>

    <div v-loading="loadingPools">
      <el-empty v-if="!pools.length" description="暂无客户公海">
        <el-button v-if="canManagePools()" type="primary" @click="router.push('/settings/customer-pools')">
          去设置创建
        </el-button>
      </el-empty>
      ...
      <template v-else>
        ...
        <div v-if="activePool" class="pool-meta">
          ...
          <span>待认领 {{ customers.length }} 条</span>
        </div>

        <el-table v-loading="loadingCustomers" :data="customers" border class="pool-table">
          <el-table-column prop="company_name" label="客户名称" min-width="160" show-overflow-tooltip>
            <template #default="{ row }">
              <el-button link type="primary" @click="goCustomer(row)">{{ row.company_name }}</el-button>
            </template>
          </el-table-column>
          <el-table-column prop="customer_number" label="客户编号" width="140" show-overflow-tooltip>
            <template #default="{ row }">{{ row.customer_number || '—' }}</template>
          </el-table-column>
        ...
        <el-empty v-if="!loadingCustomers && !customers.length" description="该公海暂无待认领客户" />
      </template>
    </div>
  </div>
</template>
