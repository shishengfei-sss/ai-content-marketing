...
async function save() {
  ...
  try {
    ...
    if (editingId.value) {
      await crmApi.updateCustomerPool(editingId.value, payload)
      ElMessage.success('公海已更新')
    } else {
      await crmApi.createCustomerPool(payload)
...
async function remove(row) {
  try {
    await ElMessageBox.confirm(
      `删除公海「${row.name}」？池内仍有未认领客户时将无法删除。`,
      '确认删除',
      { type: 'warning' },
    )
    await crmApi.deleteCustomerPool(row.id)
  ...
  <div class="page-card">
    <div class="page-head">
      <div>
        <h2>客户公海</h2>
        <p class="hint">
          维护客户公海池；退回公海后的客户会出现在
          <router-link to="/crm/customer-pools">客户管理 → 客户公海</router-link>
          ，同事可认领。
        </p>
      </div>
    ...
    >
      <el-form label-width="110px">
        <el-form-item label="名称" required>
          <el-input v-model="form.name" maxlength="100" show-word-limit placeholder="如：华东客户公海" />
        </el-form-item>
